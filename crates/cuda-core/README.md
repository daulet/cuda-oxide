...
| `ReadOnlyPageableHostMemory<'a, T>` | pageable HMM host slice | (borrowed; no cleanup call) |

## Key APIs

- **Context**: `CudaContext::new(ordinal)` retains the primary context, binds it to the calling thread, and returns an `Arc<CudaContext>`; `ctx.memory_info()` reports point-in-time free and total device capacity.
